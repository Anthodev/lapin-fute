...
- A `vX.Y.Z` tag is the only automatic production deployment trigger. After quality gates and Bunny deployment succeed, publish a GitHub Release whose first-parent commits are grouped into Features, Changes, Fixes, and CI. Merges and branch pushes must not deploy.
