// Node-side PRIM probe client and secret-redaction helpers for the catalog
// operational scripts (probe-catalog, measure-catalog, refresh-catalog).
// This is local tooling: it performs credentialed PRIM GETs only when an
// operator explicitly runs a probe, and never serves network requests.
...
export type PrimProbeErrorCode = (typeof PRIM_PROBE_ERROR)[keyof typeof PRIM_PROBE_ERROR];

const ERROR_MESSAGES: Readonly<Record<PrimProbeErrorCode, string>> = {
...
class PrimProbeError extends Error {
  readonly code: PrimProbeErrorCode;

  constructor(code: PrimProbeErrorCode) {
    super(ERROR_MESSAGES[code]);
    this.name = "PrimProbeError";
    this.code = code;
  }
}
...
export function parseBearerAuthorization(value: unknown): string {
  if (value === undefined || value === null || value === "") {
    throw new PrimProbeError(PRIM_PROBE_ERROR.API_KEY_REQUIRED);
  }
  ...
  if (
    ...
    throw new PrimProbeError(PRIM_PROBE_ERROR.API_KEY_INVALID);
  }
...
interface PrimFetchInit {
  readonly method: "GET";
  readonly headers: Readonly<{
    apikey: string;
    accept: "application/json";
    "accept-encoding": "gzip";
    "accept-language"?: WireLanguage;
  }>;
...
type PrimFetch = (target: URL, init: PrimFetchInit) => Promise<PrimFetchResponse>;
...
interface PrimRelayDependencies {
  readonly fetch: PrimFetch;
  readonly logger?: RelayLogger;
  readonly timer?: RelayTimer;
}

interface PrimRelayRequest {
  readonly target: string | URL;
  readonly authorization: unknown;
  readonly language?: WireLanguage;
}

interface PrimRelayResponse {
  readonly status: number;
...
function parsePrimTarget(value: string | URL, apiKey: string): URL {
  ...
  try {
    ...
    throw new PrimProbeError(PRIM_PROBE_ERROR.INVALID_PRIM_URL);
  }

  if (target.origin !== PRIM_ORIGIN) {
    throw new PrimProbeError(PRIM_PROBE_ERROR.PRIM_ORIGIN_REJECTED);
  }
  if (containsCredential(target, apiKey)) {
    throw new PrimProbeError(PRIM_PROBE_ERROR.PRIM_CREDENTIAL_IN_URL);
  }
...
async function readBoundedBody(
  ...
  if (typeof body.getReader !== "function") {
    throw new PrimProbeError(PRIM_PROBE_ERROR.PRIM_INVALID_RESPONSE);
  }
  ...
  try {
    while (true) {
      ...
      if (item.value.byteLength > LIMITS.httpResponseBytes - byteLength) {
        ...
        throw new PrimProbeError(PRIM_PROBE_ERROR.PRIM_RESPONSE_TOO_LARGE);
      }
...
async function performRelay(
  ...
  apiKey: string,
  fetch: PrimFetch,
  language: WireLanguage | undefined,
  controller: AbortController,
  ...
  if (isRedirect(response)) {
    throw new PrimProbeError(PRIM_PROBE_ERROR.PRIM_REDIRECT_REJECTED);
  }
...
export async function relayPrimRequest(
  ...
  const timeout = new Promise<never>((_, reject) => {
    timerHandle = timer.set(() => {
      timedOut = true;
      controller.abort();
      reject(new PrimProbeError(PRIM_PROBE_ERROR.PRIM_TIMEOUT));
  ...
  try {
    return await Promise.race([
      performRelay(target, apiKey, dependencies.fetch, request.language, controller),
      timeout,
    ]);
  } catch (failure) {
    const error = failure instanceof PrimProbeError
      ? failure
      : new PrimProbeError(
        timedOut ? PRIM_PROBE_ERROR.PRIM_TIMEOUT : PRIM_PROBE_ERROR.PRIM_REQUEST_FAILED,
      );
    log(`PRIM probe failed (${error.code})`);
    throw error;
  } finally {
    if (timerScheduled) {
      try {
