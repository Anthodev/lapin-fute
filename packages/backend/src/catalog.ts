...
import { normalizeCatalogSearchText } from "./catalog-import.ts";
...
export interface CatalogHandlerResponse {
  readonly status: 200 | 400 | 404 | 405 | 503;
  readonly body: PlaceSearchResult | ServiceOptionsResult | CatalogErrorResult;
}
...
function catalogError(code: CatalogErrorResult["code"]): CatalogErrorResult {
  return { schemaVersion: SCHEMA_VERSION, code };
}

function response(
  status: CatalogHandlerResponse["status"],
  body: CatalogHandlerResponse["body"],
): CatalogHandlerResponse {
  return { status, body };
}

function requestUrl(value: string): URL | undefined {
  try {
    return new URL(value, "http://catalog.invalid");
  } catch {
    return undefined;
  }
}

function servicePlaceId(pathname: string): string | undefined {
  const prefix = `${CATALOG_ROUTE.places}/`;
  ...
  try {
    const placeId = decodeURIComponent(encoded);
    const bytes = new TextEncoder().encode(placeId).byteLength;
    return bytes >= 1 && bytes <= LIMITS.idUtf8Bytes ? placeId : undefined;
  } catch {
    return undefined;
  }
}

function validSearchParameters(url: URL): string | undefined {
  const entries = [...url.searchParams.entries()];
  if (entries.length !== 1 || entries[0]?.[0] !== "q") return undefined;
  const query = entries[0][1];
  try {
    normalizedMatchQuery(query);
    return query;
  } catch {
    return undefined;
  }
...
export function handleCatalogRequest(
  ...
  dependencies: { readonly catalog: CatalogManager },
): CatalogHandlerResponse {
  const url = requestUrl(request.url);
  if (url === undefined) return response(400, catalogError("INVALID_QUERY"));

  if (url.pathname === CATALOG_ROUTE.places) {
    if (request.method !== "GET") return response(405, catalogError("METHOD_NOT_ALLOWED"));
    const query = validSearchParameters(url);
    if (query === undefined) return response(400, catalogError("INVALID_QUERY"));

    try {
      return response(200, {
        schemaVersion: SCHEMA_VERSION,
        places: dependencies.catalog.searchPlaces(query),
      });
    } catch (error) {
      if (error instanceof InvalidCatalogQueryError) {
        return response(400, catalogError("INVALID_QUERY"));
      }
      return response(503, catalogError("CATALOG_UNAVAILABLE"));
    }
  }

  const placeId = servicePlaceId(url.pathname);
  if (placeId !== undefined) {
    ...
    try {
      const services = dependencies.catalog.listServices(placeId);
      if (services === undefined) return response(404, catalogError("PLACE_NOT_FOUND"));
      return response(200, { schemaVersion: SCHEMA_VERSION, placeId, services });
    } catch {
      return response(503, catalogError("CATALOG_UNAVAILABLE"));
    }
  }

  return response(404, catalogError("PLACE_NOT_FOUND"));
}
