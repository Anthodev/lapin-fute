  ...
  try {
    ...
    const url = typeof urlValue === "string"
      ...
      : typeof urlValue === "object" && urlValue !== null
        ? String((urlValue as Record<string, unknown>).href ?? "")
        : "";
    let resolvedUrl: URL;
    try {
...
function prepareSpecs(database: DatabaseSync): Readonly<Record<keyof CatalogRecordSourceSet, CsvSpec>> {
  ...
  return {
    ...
    perimeter: {
      required: ["line", "name_line", "ns2_stoppointref", "ns2_stopname", "operatorname"],
      insert: (row) => perimeter.run(nonEmpty(row.line, "perimeter.line"), canonicalLineId(row.line, "perimeter.line"), nonEmpty(row.name_line, "perimeter.name_line"), nonEmpty(row.ns2_stoppointref, "perimeter.ns2_stoppointref"), nonEmpty(row.ns2_stopname, "perimeter.ns2_stopname"), nonEmpty(row.operatorname, "perimeter.operatorname")),
    },
    arrets: {
...
function createIndexes(database: DatabaseSync): void {
  database.exec(`
    ...
    CREATE INDEX stage_relations_art ON stage_relations(artid, zdaid);
...
function deriveCandidates(database: DatabaseSync): void {
  database.exec(`
    ...
    SELECT
      stop.stop_id,
      coalesce(area.zdaid, relation.zdaid),
      CASE
      ...
      END,
      CASE WHEN area.arrid IS NULL THEN 'relations' ELSE 'arrets' END
    ...
    WITH joined AS (
      SELECT DISTINCT
        ...
        CASE
          WHEN line_mode.mode IN ('RER', 'TRANSILIEN') THEN stop_zda.rail_zdaid
          ELSE stop_zda.zdaid
        END AS zdaid,
        zone.zdaname AS stop_label,
        ...
        perimeter.monitoring_ref AS monitoring_ref,
        perimeter.line AS line_ref,
        trip.direction_id AS direction_id,
        destination_code.object_code AS destination_ref,
      ...
      JOIN stage_stops AS current_stop ON current_stop.stop_id = current_stop_time.stop_id
      JOIN stage_stop_zda AS stop_zda ON stop_zda.stop_id = current_stop.stop_id
      JOIN stage_terminals AS terminal
        ON terminal.trip_id = current_stop_time.trip_id
      JOIN stage_codes AS destination_code
        ON destination_code.object_id = terminal.stop_id
        AND lower(trim(destination_code.object_system)) = 'source'
      JOIN stage_arrets_lignes AS attached
        ON attached.route_id = route.route_id
        AND attached.stop_id IN (current_stop.stop_id, current_stop.parent_station)
      JOIN stage_perimeter AS perimeter
        ON perimeter.id_line = line_mode.id_line
        AND (
          EXISTS (
            SELECT 1
            FROM stage_codes AS monitoring_code
            WHERE monitoring_code.object_id IN (current_stop.stop_id, current_stop.parent_station)
              AND monitoring_code.object_code = perimeter.monitoring_ref
          )
          OR instr(perimeter.monitoring_ref, ':' || stop_zda.rail_zdaid || ':') > 0
        )
    ...
    FROM joined
    WHERE zdaid IS NOT NULL AND trim(zdaid) <> ''
    GROUP BY mode, zdaid, monitoring_ref, id_line, line_ref, direction_id, destination_ref;
  `);
}
  ...
  try {
    ...
    for (const row of candidates) {
      ...
      const service = createServiceIdentity({
        ...
        destinationRef: nonEmpty(row.destination_ref, "candidate.destination_ref"),
      });
...
function validateOpenCatalog(database: DatabaseSync): CatalogBuildResult {
  ...
  return { candidatePath: "", sourceRevision, createdAt, placeCount, serviceCount, countsByMode: counts };
}
  ...
  const missing = database.prepare(`
    ...
    WHERE NOT EXISTS (
      ...
      WHERE perimeter.line = candidate.line_ref
        AND perimeter.monitoring_ref = candidate.monitoring_ref
    )
    LIMIT 1
  `).get();
  if (missing !== undefined) fail("PERIMETER_MISMATCH", "offered service lacks its exact perimeter pair");
}
...
function validateCandidateSemanticTuples(database: DatabaseSync): void {
  const ambiguous = database.prepare(`
    SELECT 1
    FROM stage_candidates
    GROUP BY mode, id_line, monitoring_ref, direction_id, destination_ref
